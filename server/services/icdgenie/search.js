...
async function opensearch(request, response) {
  ...
  const [tabularResult, neoplasmResult, drugResult, injuryResult, icdo3Result, icd11Result] = await Promise.all([
  ...
  if (results.tabular.length || results.neoplasm.length || results.drug.length || results.injury.length || results.icdo3.length || results.icd11.length) {
    const [tabularFuzzy, neoplasmFuzzy, drugFuzzy, injuryFuzzy, icdo3Fuzzy, icd11Fuzzy] = await Promise.all([
